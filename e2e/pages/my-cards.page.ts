...
export class MyCardsPage {
    ...
    async gotoWithMock() {
        ...
        await expect(this.page.getByTestId('card-skeleton').first()).not.toBeVisible();
        await expect(this.page.locator('[data-testid^="card-"]').first()).toBeVisible({ timeout: 10000 });
    }
    ...
    async gotoEmpty() {
        ...
        await this.page.route('**/api/collections/progress*', async (route: Route) => {
            await route.fulfill({ json: { data: [] } });
        });
        await this.page.goto('/my-cards');
        // Wait for empty state specifically
        await expect(this.page.getByTestId('empty-state')).toBeVisible({ timeout: 10000 });
    }
