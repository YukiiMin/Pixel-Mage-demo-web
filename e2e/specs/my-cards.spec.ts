...
test.describe('My Cards Page', () => {
    ...
    test('shows skeleton loading state initially', async ({ page }) => {
        ...
        const gotoPromise = myCardsPage.page.goto('/my-cards', { waitUntil: 'domcontentloaded' });
    ...
    test('card grid shows correct number of cards with data', async () => {
        ...
        await expect(myCardsPage.cardGrid.locator('[data-testid^="card-"]')).toHaveCount(3);
    });
    ...
    test('filtering functionally works and shows filter-empty-state when no match', async () => {
        ...
        await expect(myCardsPage.cardGrid.locator('[data-testid^="card-"]')).toHaveCount(1, { timeout: 10000 });
        ...
        await expect(myCardsPage.cardGrid.locator('[data-testid^="card-"]')).toHaveCount(3, { timeout: 10000 });
    });
