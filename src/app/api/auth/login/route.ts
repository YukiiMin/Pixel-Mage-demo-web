...
function normalizeLoginPayload(payload: unknown): {
  ...
  const account =
    pickRecord(raw.account) ?? pickRecord(raw.user) ?? pickRecord(data?.account) ?? pickRecord(data?.user) ?? null;
  ...
  let name =
    String(account?.name ?? account?.fullName ?? raw.name ?? data?.name ?? "").trim() || null;
