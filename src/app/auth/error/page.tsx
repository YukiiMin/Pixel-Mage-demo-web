...
function AuthErrorContent() {
  ...
  const errorTitle = searchParams.get("error") === "email_not_verified" 
    ? "Email chưa được xác thực" 
    : "Lỗi đăng nhập";
  const errorMessage = searchParams.get("message") || "Đã xảy ra lỗi trong quá trình đăng nhập. Vui lòng thử lại.";
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md px-6 py-24 text-center">
        <Suspense fallback={<div className="text-muted-foreground">Đang tải chi tiết lỗi...</div>}>
          <AuthErrorContent />
        </Suspense>
      </div>
    </div>
