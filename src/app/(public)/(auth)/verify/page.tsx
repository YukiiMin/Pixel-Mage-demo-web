		...
		const processVerification = async () => {
			try {
				...
				const res = await fetch(
					`/api/accounts/auth/verify?token=${encodeURIComponent(token)}`,
				);
				...
				if (res.ok) {
					...
					setMessage(
						data.message ||
							"Xác thực email thành công! Bạn có thể bắt đầu đăng nhập.",
					);
				} else {
					setStatus("error");
					setMessage(
						data.message || "Xác thực email thất bại. Token có thể đã hết hạn.",
					);
				}
			} catch (err) {
				setStatus("error");
				setMessage(
					"Đã xảy ra sự cố trong quá trình xác thực. Vui lòng thử lại sau.",
				);
			}
		};
