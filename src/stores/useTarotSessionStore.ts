...
export type SessionPhase = "SHUFFLING" | "SELECTING" | "REVEALING" | "INTERPRETING" | "COMPLETE";
...
export const useTarotSessionStore = create<TarotSessionState>((set, get) => ({
  ...
  setSpreadType: (s) => set((st) => ({ setup: { ...st.setup, spreadType: s } })),
  setDeckMode: (m) => set((s) => ({ setup: { ...s.setup, deckMode: m } })),

  phase: "SHUFFLING",
  setPhase: (p) => set({ phase: p }),
  selectedCards: [],
  addCard: (slot) => set((s) => ({ selectedCards: [...s.selectedCards, slot] })),
  revealedCount: 0,
