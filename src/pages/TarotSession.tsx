...
import { useTarotSessionStore, SessionPhase } from "@/stores/useTarotSessionStore";
...
/* ── Phase 1: Shuffling ── */
const ShufflingPhase = ({ onDone }: { onDone: () => void }) => {
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">Hãy tập trung vào câu hỏi của bạn</p>
      </div>
      {/* progress */}
  ...
  const handlePick = (cardData: (typeof MAJOR_ARCANA)[0]) => {
    ...
    addCard({
      ...
      card: {
        ...
        image: "",
        isReversed,
        meaning: isReversed ? cardData.reversedMeaning : cardData.uprightMeaning,
      },
    });
  };
  ...
  return (
    ...
    >
      <div className="text-center space-y-1">
        <h2 className="text-2xl font-bold text-foreground">Chọn {required} lá bài</h2>
        <p className="text-sm text-muted-foreground">Hãy để trực giác dẫn lối — bấm vào lá bạn cảm nhận</p>
      </div>
        ...
        {deck.map((c, i) => {
          ...
          return (
            <motion.button
              ...
              animate={{ opacity: picked ? 0.3 : 1, y: 0, scale: picked ? 0.9 : 1 }}
            ...
            >
              {filled ? (
                ...
                >
                  ...
                  <p className="text-[10px] text-primary font-medium mt-1">Lá {i + 1}</p>
                </motion.div>
              ) : (
...
/* ── Phase 3: Revealing ── */
const RevealingPhase = ({ onDone }: { onDone: () => void }) => {
  const { selectedCards, revealedCount, setRevealedCount } = useTarotSessionStore();
  ...
  return (
    <motion.div
      ...
      className="space-y-8"
    >
      <h2 className="text-2xl font-bold text-center text-foreground">Lật Bài...</h2>
              ...
              >
                ...
                <div
                  className={`absolute inset-0 rounded-xl border-2 flex flex-col items-center justify-center gap-2 p-3 ${
                    revealed ? "glow-gold border-primary bg-card" : "border-primary/40 bg-card"
                  }`}
                ...
                >
                  ...
                  <p className="text-[10px] text-muted-foreground text-center">{cardInfo?.name}</p>
...
const InterpretingPhase = ({ onDone }: { onDone: () => void }) => {
  const { selectedCards, interpretation, setInterpretation } = useTarotSessionStore();
  ...
  return (
    ...
    >
        ...
        {selectedCards.map((slot, i) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-xs font-bold text-foreground">{card?.nameVi}</p>
              {slot.card.isReversed && (
                <span className="text-[9px] text-destructive font-bold">REVERSED</span>
              )}
            </motion.div>
          );
        })}
      ...
      >
        ...
        <Button variant="outline" className="rounded-full px-6 border-primary/40 text-primary hover:bg-primary/10">
          <Share2 className="w-4 h-4 mr-1" /> Chia Sẻ
        </Button>
        <Button variant="ghost" onClick={handleRedo} className="rounded-full px-6 text-muted-foreground hover:text-foreground">
...
const TarotSession = () => {
  ...
  const phases: SessionPhase[] = ["SHUFFLING", "SELECTING", "REVEALING", "INTERPRETING", "COMPLETE"];
      ...
      <main className="relative z-10 pt-28 pb-20">
        <div className="container mx-auto px-6 max-w-3xl">
          ...
          <div className="flex items-center justify-center gap-1 mb-10">
            {phases.map((p, i) => (
              <div key={p} className="flex items-center gap-1">
                ...
                {i < phases.length - 1 && (
                  <div className={`w-4 h-0.5 ${i < idx ? "bg-primary/40" : "bg-muted"}`} />
                )}
              </div>
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            {phase === "SHUFFLING" && <ShufflingPhase key="shuffle" onDone={goTo("SELECTING")} />}
            {phase === "SELECTING" && (
              <SelectingPhase key="select" required={required} onComplete={goTo("REVEALING")} />
            )}
            {phase === "REVEALING" && <RevealingPhase key="reveal" onDone={goTo("INTERPRETING")} />}
            {phase === "INTERPRETING" && <InterpretingPhase key="interp" onDone={goTo("COMPLETE")} />}
