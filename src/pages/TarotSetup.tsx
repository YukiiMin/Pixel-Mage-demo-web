...
import { useTarotSessionStore, TarotTopic, SpreadType } from "@/stores/useTarotSessionStore";
import { TOPICS, SPREADS } from "@/lib/tarot-data";
import { Textarea } from "@/components/ui/textarea";
import { Button } from "@/components/ui/button";
import { Switch } from "@/components/ui/switch";
import { fadeInUp, staggerContainer } from "@/lib/motion-variants";
import { ArrowRight, ArrowLeft, Sparkles, History, AlertTriangle } from "lucide-react";
...
const TarotSetup = () => {
  ...
  const handleStart = () => {
    ...
    navigate(`/tarot/session/${sessionId}`);
  };
        ...
        <div className="container mx-auto px-6 max-w-2xl">
          ...
          <div className="flex items-center justify-center gap-2 mb-10">
            {steps.map((s, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                {i < steps.length - 1 && (
                  <div className={`w-12 h-0.5 transition-colors duration-300 ${i < step ? "bg-primary/50" : "bg-muted"}`} />
                )}
              </div>
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 0 && (
              ...
              >
                <motion.h2 variants={fadeInUp} className="text-3xl font-bold text-center">
                  Bạn muốn hỏi về <span className="gradient-gold-purple">điều gì?</span>
                </motion.h2>
                <motion.div variants={fadeInUp} className="grid grid-cols-2 gap-4">
                  {TOPICS.map((t) => (
                    ...
                    >
                      <span className="text-4xl block mb-3 group-hover:scale-110 transition-transform">{t.emoji}</span>
            ...
            {step === 1 && (
              ...
              >
                ...
                <motion.p variants={fadeInUp} className="text-center text-muted-foreground text-sm">
                ...
                <motion.div variants={fadeInUp}>
                  <Textarea
                    ...
                    maxLength={500}
                  />
                  <p className="text-right text-xs text-muted-foreground mt-1">{store.setup.question.length}/500</p>
                </motion.div>
              </motion.div>
            )}
                  ...
                  {SPREADS.map((sp) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="font-bold text-lg text-foreground">{sp.label}</span>
                          <p className="text-sm text-muted-foreground mt-0.5">{sp.description}</p>
                        </div>
                        <span className="text-2xl font-bold text-primary">{sp.count}</span>
                      </div>
                    </button>
                  ))}
                </motion.div>
                ...
                <motion.div variants={fadeInUp} className="glass-card rounded-xl p-5">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-xs font-medium ${store.setup.deckMode === "EXPLORE" ? "text-primary" : "text-muted-foreground"}`}>
                        EXPLORE
                      </span>
                      <Switch
                        checked={store.setup.deckMode === "YOUR_DECK"}
                        onCheckedChange={(c) => store.setDeckMode(c ? "YOUR_DECK" : "EXPLORE")}
                      />
                      <span className={`text-xs font-medium ${store.setup.deckMode === "YOUR_DECK" ? "text-primary" : "text-muted-foreground"}`}>
                        YOUR DECK
                      </span>
                    </div>
                  </div>
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Tarot là công cụ suy ngẫm, không phải lời tiên tri. Kết quả chỉ mang tính tham khảo, hãy luôn dùng phán đoán của riêng bạn cho các quyết định quan trọng.
              </p>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="glass-card rounded-xl p-8 text-center">
              <p className="text-muted-foreground text-sm">Chưa có lịch sử đọc bài. Bắt đầu trải bài đầu tiên nào!</p>
            </div>
          </motion.div>
        </div>
      </main>
