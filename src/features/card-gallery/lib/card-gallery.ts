...
export async function getCardFrameworks(): Promise<CardFramework[]> {
	try {
		const response = await apiRequest<CardFrameworkResponse[]>(
			"/api/card-frameworks",
		);
...
export async function getCardTemplates(
	...
	try {
		...
		const response = await apiRequest<
			PageResponse<CardTemplateResponseSummary>
		...
		return {
			...
			numberOfElements:
				response.data?.numberOfElements ?? transformedData.length,
		};
	} catch (error) {
...
export async function getCardTemplateById(
	id: number,
): Promise<CardTemplateWithContent | null> {
	try {
		const response = await apiRequest<CardTemplateResponse>(
...
// Card Content API
// ─────────────────────────────────────────────
export async function getCardContentsByTemplateId(
	templateId: number,
): Promise<CardContent[]> {
	try {
		const response = await apiRequest<CardContentResponse[]>(
