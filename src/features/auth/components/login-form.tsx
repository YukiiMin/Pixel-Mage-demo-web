...
import { AlertCircle, Eye, EyeOff, LoaderCircle, LogIn, Sparkles } from "lucide-react";
...
const containerVariants: Variants = {
	hidden: { opacity: 0 },
	visible: { opacity: 1, transition: { delayChildren: 0.1, staggerChildren: 0.09 } },
};
...
export function LoginForm() {
	...
	return (
		<div className="group relative w-full overflow-hidden rounded-[20px] p-px">
			...
			<div className="absolute -inset-full animate-[spin_4s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#FFD700_0%,#393BB2_50%,#FFD700_100%)] opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
			...
			>
				...
				<motion.div variants={itemVariants} className="mb-5 flex items-center gap-3">
					<div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl gradient-gold-purple-bg shadow-[0_0_16px_rgba(234,179,8,0.3)]">
					...
					<div>
						...
						<p className="mt-0.5 text-xs text-muted-foreground">
							Đồng bộ dữ liệu với tài khoản PixelMage
						</p>
					</div>
				</motion.div>
				...
				<motion.div variants={itemVariants} className="space-y-1">
					<Input
						...
						placeholder="Email"
						autoComplete="email"
						className="h-11 bg-background/50 backdrop-blur-sm transition-all duration-300 focus-visible:ring-2 focus-visible:ring-yellow-500/50"
					/>
					{errors.email && (
				...
				<motion.div variants={itemVariants} className="relative mt-3 space-y-1">
					<div className="relative">
						<Input
							...
							className="h-11 pr-11 bg-background/50 backdrop-blur-sm transition-all duration-300 focus-visible:ring-2 focus-visible:ring-yellow-500/50"
						/>
						<button
							type="button"
				...
				<motion.div variants={itemVariants} className="mt-2 flex justify-end">
					<Link
						...
						className="text-xs font-medium text-muted-foreground transition-colors hover:text-yellow-400"
					>
						Quên mật khẩu?
					</Link>
				</motion.div>
				...
				{justRegistered && (
					<motion.p variants={itemVariants} className="mt-3 text-xs font-medium text-emerald-400">
				...
				>
					<Button
						type="submit"
						disabled={loading}
						className="h-11 w-full gradient-gold-purple-bg text-primary-foreground shadow-[0_0_15px_rgba(234,179,8,0.2)] transition-shadow duration-300 hover:shadow-[0_0_25px_rgba(234,179,8,0.4)]"
					>
						{loading ? (
