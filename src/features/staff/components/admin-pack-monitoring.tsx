...
export function AdminPackMonitoring() {
  ...
  const [statusFilter, setStatusFilter] = useState<'ALL' | 'STOCKED' | 'RESERVED' | 'SOLD'>('ALL')
  ...
  const fetchPacks = useCallback(async () => {
    ...
    try {
      ...
      const mockPacks: Pack[] = Array.from({ length: 50 }, (_, i) => ({
        ...
        product: {
          productId: (i % 3) + 1,
          name: ['PixelMage Standard Pack', 'Major Sealed Box', 'PixelMage Blister Promo'][i % 3],
        ...
        packDetails: Array.from({ length: 5 }, (_, j) => ({
          ...
          card: {
            ...
            linkedAt: j < 2 ? new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString() : undefined,
            soldAt: j < 4 ? new Date(Date.now() - Math.random() * 14 * 24 * 60 * 60 * 1000).toISOString() : undefined,
            createdAt: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(),
            updatedAt: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString()
          }
        })),
        order: ['RESERVED', 'SOLD'].includes(['STOCKED', 'RESERVED', 'SOLD'][i % 3]) ? {
          orderId: i + 1,
          customer: {
            accountId: Math.floor(Math.random() * 100) + 1,
            name: `Customer ${Math.floor(Math.random() * 100) + 1}`,
      ...
      const mockStats: PackMonitoringStats = {
        ...
        linkedCards: mockPacks.reduce((acc, p) =>
          acc + p.packDetails.filter(pd => pd.card.status === 'LINKED').length, 0),
        pendingCards: mockPacks.reduce((acc, p) =>
          acc + p.packDetails.filter(pd => pd.card.status === 'READY').length, 0),
        soldCards: mockPacks.reduce((acc, p) =>
          acc + p.packDetails.filter(pd => pd.card.status === 'SOLD').length, 0)
      }
      setStats(mockStats)
    } catch (err) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'STOCKED': return 'bg-blue-500/10 text-blue-400 border-blue-500/25'
      case 'RESERVED': return 'bg-yellow-500/10 text-yellow-400 border-yellow-500/25'
      case 'SOLD': return 'bg-green-500/10 text-green-400 border-green-500/25'
  ...
  const getCardStatusColor = (status: string) => {
    switch (status) {
      case 'PENDING_BIND': return 'bg-gray-500/10 text-gray-400 border-gray-500/25'
      case 'READY': return 'bg-blue-500/10 text-blue-400 border-blue-500/25'
      case 'SOLD': return 'bg-yellow-500/10 text-yellow-400 border-yellow-500/25'
      case 'LINKED': return 'bg-green-500/10 text-green-400 border-green-500/25'
      case 'DEACTIVATED': return 'bg-red-500/10 text-red-400 border-red-500/25'
      default: return 'bg-muted/40 text-muted-foreground'
    }
  }
  ...
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      case 'LEGENDARY': return 'bg-gradient-to-r from-yellow-500/20 to-orange-500/20 text-yellow-400 border-yellow-500/30'
      case 'RARE': return 'bg-gradient-to-r from-blue-500/20 to-purple-500/20 text-blue-400 border-blue-500/30'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground" style={{ fontFamily: 'Fruktur, var(--font-heading)' }}>
            Pack Monitoring
          </h1>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-400">{stats.stockedPacks}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-yellow-400">{stats.reservedPacks}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-400">{stats.soldPacks}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="glass-card">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">Thẻ đã liên kết</p>
                  <p className="text-xl font-bold text-green-400">{stats.linkedCards}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-blue-400">{stats.pendingCards}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-yellow-400">{stats.soldCards}</p>
                </div>
              </div>
            </CardContent>
          </Card>
                  ...
                  <TableRow key={pack.packId} className="hover:bg-card/40">
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">#{pack.packId}</p>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge className={getStatusColor(pack.status)}>
                        {pack.status === 'STOCKED' && <Box className="h-3 w-3 mr-1" />}
                        {pack.status === 'RESERVED' && <Clock className="h-3 w-3 mr-1" />}
                        {pack.status === 'SOLD' && <CheckCircle2 className="h-3 w-3 mr-1" />}
                        {pack.status}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <div className="text-sm">
                        ...
                        <p className="text-muted-foreground mt-1">
                          {pack.packDetails.filter(pd => pd.card.status === 'LINKED').length} đã liên kết
                        </p>
                      </div>
                    </TableCell>
                    <TableCell>
                      {pack.order ? (
                        <div className="text-sm">
                          <p className="font-medium">{pack.order.customer.name}</p>
                          <p className="text-muted-foreground">{pack.order.customer.email}</p>
                        </div>
                      ) : (
                    ...
                    <TableCell>
                      <div className="text-sm">
                        ...
                        <p className="text-muted-foreground">by {pack.createdBy.name}</p>
                      </div>
                    </TableCell>
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedPack && (
            <div className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-bold text-lg">{selectedPack.product.name}</h3>
                        <p className="text-2xl font-bold text-primary">{formatCurrency(selectedPack.product.price)}</p>
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="text-center p-3 bg-muted/30 rounded-lg">
                        <p className="text-2xl font-bold">{selectedPack.packDetails.length}</p>
                        <p className="text-sm text-muted-foreground">Tổng thẻ</p>
                      </div>
                      <div className="text-center p-3 bg-green-500/10 rounded-lg">
                        <p className="text-2xl font-bold text-green-400">
                          {selectedPack.packDetails.filter(pd => pd.card.status === 'LINKED').length}
                        </p>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">Chi tiết thẻ trong Pack</CardTitle>
                </CardHeader>
                <CardContent>
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {selectedPack.packDetails.map((packDetail) => (
                        <TableRow key={packDetail.packDetailId}>
                          <TableCell>
                            <Badge variant="outline">#{packDetail.positionIndex}</Badge>
                          </TableCell>
                          <TableCell>
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-medium text-sm">{packDetail.card.cardTemplate.name}</p>
                              </div>
                            </div>
                          </TableCell>
                          <TableCell>
                            <Badge className={getRarityColor(packDetail.card.cardTemplate.rarity)}>
                              {packDetail.card.cardTemplate.rarity}
                            </Badge>
                          </TableCell>
                          <TableCell>
                            <Badge className={getCardStatusColor(packDetail.card.status)}>
                              {packDetail.card.status === 'LINKED' && <Smartphone className="h-3 w-3 mr-1" />}
                              {packDetail.card.status === 'READY' && <PackageOpen className="h-3 w-3 mr-1" />}
                              {packDetail.card.status === 'SOLD' && <ShoppingCart className="h-3 w-3 mr-1" />}
                          ...
                          <TableCell>
                            {packDetail.card.owner ? (
                              <div className="text-sm">
                                <p className="font-medium">{packDetail.card.owner.name}</p>
                                <p className="text-muted-foreground">{packDetail.card.owner.email}</p>
                              </div>
                            ) : (
                              <span className="text-muted-foreground">—</span>
                            )}
                          ...
                          <TableCell>
                            {packDetail.card.linkedAt ? (
                              <span className="text-sm">{formatDate(packDetail.card.linkedAt)}</span>
                            ) : (
                              <span className="text-muted-foreground">—</span>
                            )}
                          </TableCell>
