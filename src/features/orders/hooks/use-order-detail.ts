...
function normalizeOrderDetail(
	...
	const fallbackItemsSource =
		orderItemsPayload && typeof orderItemsPayload === "object"
			? ((orderItemsPayload as Record<string, unknown>).data ??
				orderItemsPayload)
			: orderItemsPayload;
	...
	return {
		...
		customerId:
			String(raw.customerId ?? raw.accountId ?? "").trim() || undefined,
		items,
	};
}
...
export function useOrderDetail(orderId: string) {
	...
	useEffect(() => {
		...
		const load = async () => {
			...
			try {
				const [ordersByCustomerResponse, orderItemsResponse] =
					await Promise.all([
						apiRequest<unknown>(
				...
				const orderListSource = Array.isArray(ordersByCustomerResponse.data)
					...
					: ordersByCustomerResponse.data &&
						...
						? ((
								ordersByCustomerResponse.data as {
									data?: unknown;
									items?: unknown;
								}
							).data ??
				...
				if (!normalized) {
					...
					setStatus("unavailable");
					setStatusMessage(
						"BE đã phản hồi nhưng dữ liệu đơn hàng chưa đầy đủ. Chức năng chưa cập nhật.",
					);
					return;
				}
				...
				setStatus("unavailable");
				setStatusMessage(
					`${getApiErrorMessage(error, "Không thể tải đơn hàng từ BE.")} Chức năng chưa cập nhật.`,
				);
			}
		};
