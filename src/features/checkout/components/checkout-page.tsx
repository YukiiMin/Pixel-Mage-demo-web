...
export function CheckoutPage({ productId }: Props) {
	...
	useEffect(() => {
		...
		if (!ct) {
			...
			setErrorMessage(
				"Không có checkout token. Vui lòng quay lại ứng dụng và thử lại.",
			);
			return;
		}
		...
		(async () => {
			try {
				...
				setStep("invalid_token");
				setErrorMessage(
					"Checkout token không hợp lệ hoặc đã hết hạn (5 phút). Vui lòng quay lại app và thử lại.",
				);
			}
		})();
	}, [ct]);
	...
	useEffect(() => {
		...
		if (orderStatus.paymentStatus === "SUCCEEDED") {
			...
			setTimeout(
				() => redirectToMobile("success", { orderId: String(orderId) }),
				2500,
			);
		} else if (
		...
		<div className="container mx-auto max-w-lg px-4 py-10">
			...
			<div className="mb-8 flex items-center gap-3">
				<button
					type="button"
					onClick={() =>
						redirectToMobile("failed", { reason: "user_cancelled" })
					}
			...
			{step === "invalid_token" && (
				<StateCard error>
					...
					<button
						type="button"
						onClick={() =>
							redirectToMobile("failed", { reason: "invalid_token" })
						}
				...
				<div className="space-y-6">
					...
					<div className="glass-card rounded-2xl border border-border/40 p-6">
						{pack.poolPreview && pack.poolPreview.length > 0 && (
							...
							<img
								src={
									pack.poolPreview[0]?.imagePath ||
									"https://placehold.co/800x400?text=Product"
								}
								alt={pack.name}
					...
					<p className="text-center text-xs text-muted-foreground">
						Bạn sẽ thấy mã QR VietQR sau khi xác nhận. Quét bằng ứng dụng ngân
						hàng.
					</p>
				</div>
			)}
