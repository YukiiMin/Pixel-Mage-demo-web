...
import type { ProductSort } from "@/features/marketplace/hooks/use-marketplace";
import type { ProductCategory, Rarity } from "@/types/commerce";
		...
		<section className="glass-card rounded-2xl border-border/50 p-5 md:p-6">
			...
			<div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
				...
				<select
					value={rarity}
					onChange={(event) =>
						onRarityChange(event.target.value as Rarity | "all")
					}
					className="h-10 rounded-md border border-input bg-background px-3 py-2 text-sm"
				>
