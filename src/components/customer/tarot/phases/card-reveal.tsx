...
export function CardReveal({ onAllRevealed }: CardRevealProps) {
	...
	const setRevealedCount = useTarotSessionStore((state) => state.setRevealedCount);
	...
	return (
		<motion.div
			...
			className="space-y-8"
		>
			<h2 className="text-center text-2xl font-bold text-foreground">Lật Bài...</h2>
			<div className="flex flex-wrap justify-center gap-6">
				{selectedCards.map((slot, index) => {
					const isRevealed = index < revealedCount;
					const cardMeta = MAJOR_ARCANA.find((card) => card.id === slot.card.id);
					return (
						<div key={slot.card.id} className="relative" style={{ perspective: 800 }}>
							<motion.div
							...
							>
								...
								>
									...
									<p className="text-center text-[10px] text-muted-foreground">{cardMeta?.name}</p>
