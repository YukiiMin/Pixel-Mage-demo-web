	...
	return (
		<div className="container mx-auto max-w-3xl px-6">
				...
				{phaseOrder.map((phaseName, index) => (
					<div key={phaseName} className="flex items-center gap-1">
						...
						{index < phaseOrder.length - 1 && (
							<div className={`h-0.5 w-4 ${index < activePhaseIndex ? "bg-primary/40" : "bg-muted"}`} />
						)}
					</div>
				))}
			</div>
			...
			<AnimatePresence mode="wait">
				{phase === "SHUFFLING" && <ShufflingPhase key="shuffle" onComplete={() => goTo("SELECTING")} />}
				{phase === "SELECTING" && (
					<CardDrawArea
						key="select"
				...
				{phase === "INTERPRETING" && (
					<AIStreamPanel key="interpreting" onStreamComplete={() => goTo("COMPLETE")} />
				)}
