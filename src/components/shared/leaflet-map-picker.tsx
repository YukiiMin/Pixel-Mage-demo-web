...
export function LeafletMapPicker({
	...
	useEffect(() => {
		...
		function initMap() {
			...
			const marker = L.marker(defaultLocation, { draggable: true, icon }).addTo(map);
	...
	return (
		<div className="w-full flex flex-col gap-3">
			...
			<div className="relative w-full z-20 flex gap-2">
				...
				>
					{isSearching ? <LoaderCircle className="h-4 w-4 animate-spin" /> : "Tìm kiếm"}
				</Button>
					...
					<div className="absolute top-11 left-0 right-[108px] bg-card border border-border/50 shadow-xl rounded-lg overflow-hidden max-h-60 overflow-y-auto">
						{searchResults.map((item, idx) => (
							...
							>
								...
								<p className="text-sm text-foreground line-clamp-2">{item.display_name}</p>
							</div>
						))}
					</div>
				)}
